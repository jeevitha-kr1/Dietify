...
import { saveUserAnswers } from "../api/profile";

const KEY = "dietify_user_inputs_v1";
const SKIPPED = "__skipped__";
const PROFILE_READY_KEY = "dietify_profile_ready_v1";
...
export default function UserInput() {
  ...
  const canGoNext = () =>
    !q?.required ? true : !isEmpty(answers[q.id]) && !isSkipped(answers[q.id]);
  ...
  const skippedCount = useMemo(
    () => summaryRows.filter((r) => r.skipped).length,
    [summaryRows]
  );
  const firstSkipped = useMemo(
    () => summaryRows.find((r) => r.skipped),
    [summaryRows]
  );
    ...
    if (qq.type === "multi") {
      setEditValue(
        Array.isArray(current)
          ? current
          : isSkipped(current) || isEmpty(current)
          ? []
          : [String(current)]
      );
    } else {
  ...
  const saveEdit = () => {
    if (!editQ) return;
    if (editQ.required && (isEmpty(editValue) || isSkipped(editValue))) return;

    if (editQ.type === "multi") {
      const arr = Array.isArray(editValue) ? editValue : [];
      if (arr.includes("None") && arr.length > 1) setAnswer(editQ.id, ["None"]);
      else setAnswer(editQ.id, arr);
      closeEdit();
      return;
    }
  ...
  // ✅ Save to backend and go to Dashboard
  const confirmAndContinue = async () => {
    try {
      ...
      // mark onboarding complete for dashboard UX
      localStorage.setItem(PROFILE_READY_KEY, "1");

      navigate("/dashboard"); // ✅ changed
    } catch (e) {
      console.error("SAVE ANSWERS FAILED:", e);
      setSaveError(`Could not save your profile. ${e?.message || ""}`.trim());
    } finally {
      setSaving(false);
    }
  };
        ...
        {stage === "result" && (
          <div className="ui-card">
            ...
            <div className="ui-actions ui-result-actions">
              ...
              <button className="ui-primary" type="button" onClick={confirmAndContinue} disabled={saving}>
        ...
        <div className="ui-modalOverlay" role="dialog" aria-modal="true" aria-label="Edit answer">
          <div className="ui-modal">
            <div className="ui-modalTop">
              ...
              <button className="ui-modalClose" type="button" onClick={closeEdit} aria-label="Close">✕</button>
            </div>
            ...
            <div className="ui-modalActions">
              <div className="ui-modalLeft">
                {!editQ.required && (
                  <>
                    <button className="ui-ghost" type="button" onClick={clearAnswer}>Clear</button>
                    <button className="ui-ghost" type="button" onClick={markSkipped}>Skip</button>
                  </>
                )}
              </div>

              <div className="ui-modalRight">
                <button className="ui-secondary" type="button" onClick={closeEdit}>Cancel</button>
                <button className="ui-primary" type="button" onClick={saveEdit}>Save</button>
              </div>
            </div>
