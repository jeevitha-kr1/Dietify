...
function buildStore(aiOverride = {}, profileOverride = mockProfile) {
...
function renderResult(aiOverride = {}, profileOverride = mockProfile) {
  const store = buildStore(aiOverride, profileOverride);
  return render(
...
test("page heading shows up", () => {
  renderResult();
  expect(
    screen.getByText(/your personalized nutrition plan/i)
  ).toBeInTheDocument();
});
...
test("AI summary text renders", () => {
  renderResult();
  expect(
    screen.getByText(/your personalized plan is ready/i)
  ).toBeInTheDocument();
});
...
test("add ingredients button is visible", () => {
  ...
  const addButtons = screen.getAllByRole("button", {
    name: /add ingredients to cart/i,
  });
...
test("shows fallback message when no tips available", () => {
  renderResult({ tips: [] });
  expect(
    screen.getByText(/no additional tips available yet/i)
  ).toBeInTheDocument();
});
...
test("shows fallback data when AI call fails", () => {
  renderResult({
    weeklyPlan: mockWeeklyPlan,
    summary: "AI is temporarily unavailable. Showing a demo meal plan instead.",
  });
  expect(
...
test("shows no meal plan when AI returns empty days", () => {
  renderResult({ weeklyPlan: [] });
  expect(
    screen.getByText(/no weekly meal plan available yet/i)
  ).toBeInTheDocument();
...
test("shows no meals message when day has empty meals array", () => {
  ...
  expect(
    screen.getByText(/no meals available for this day/i)
  ).toBeInTheDocument();
});